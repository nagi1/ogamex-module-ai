...
INSERT OR REPLACE INTO tasks
 ...
 (7,'IMPL-018','Fleet composition — U-series production + launch subset, counters, recyclers','impl','todo','P1','W6-4',
  'FLE-002,FLE-004,FLE-012,FLE-013,FLE-014,FLE-015','U-series (block to write)','specs/gameplay-algorithms.md (U1-U4); research/strategy/principles/fleet-composition.yaml',
 ...
 (13,'DISC-001','Re-source ACS tutorials ORG-009/010','discovery','open','P3',NULL,NULL,NULL,
  'research/strategy/sources.yaml',NULL,'Wayback failed; GF-003 suffices. Non-blocking.'),
 (14,'DISC-002','French board guide library','discovery','open','P3',NULL,NULL,NULL,
  'research/strategy/sources.yaml',NULL,'Confirmed to exist; thread URLs not retrievable. Non-blocking.'),
 (15,'DISC-003','ogamewiki.de','discovery','open','P3',NULL,NULL,NULL,
  'research/strategy/sources.yaml',NULL,'Extraction failed. Non-blocking.'),
 ...
 (17,'DOC-001','Write U-series fleet-composition algorithm blocks','doc','todo','P1','W6-4',
  ...
  'specs/gameplay-algorithms.md (U1-U4); research/strategy/principles/fleet-composition.yaml',
 ...
 (18,'DOC-002','Write ninja algorithm block','doc','todo','P2','pass-6 (new)','NIN-001..005','N-series (new)',
  'research/strategy/principles/ninja-baiting.yaml; research/architecture-mapping.md (pass-6)',
  'plan/details/specs/gameplay-algorithms.md',
 ...
 (19,'DOC-003','Write expedition algorithm block','doc','todo','P2','pass-6 (new)','EXP-001,EXP-002','E-series (new)',
  'research/strategy/principles/expeditions.yaml',
  'plan/details/specs/gameplay-algorithms.md',
 ...
 (26,'IMPL-025','Scarcity-weighted resource need: the scarce resource ranks above the persona habit','impl','todo','P2','W7-1',NULL,'E6',
  'specs/gameplay-algorithms.md (E6)',
  'app/Domain/Decision/CandidateActionFactory.php',
  'The other half of W7-1: a fleeter ranks ships 15 points above mines in every session. Give Build a scarcity-weighted resource_need so it wins exactly when a resource is the binding constraint; frozen-clock before/after.'),
 (27,'IMPL-026','Objective resolution from a committed coalition victory','impl','todo','P2',NULL,NULL,NULL,
  'specs/pve-empire.md',
  'app/Actions/ResolveAiCampaignObjectiveFromBattleReportAction.php; app/Observers/ObserveCommittedBattleReport.php; app/Models/AiCampaignObjective.php',
  'A committed battle report whose attacker wins against a declared objective planet marks that objective complete once. A draw, retreat or defender win does not count; winning does not capture the planet.'),
 (28,'IMPL-027','Campaign director lifecycle','impl','todo','P2',NULL,NULL,NULL,
  'specs/pve-empire.md',
  'app/Actions/AdvanceAiCampaignStateAction.php; app/Models/AiCampaign.php',
  'Preparing->Active at starts_at; Active->Resolved when every declared objective completes on time; Failed when the deadline passes with an incomplete set. Idempotent terminal states.'),
 (29,'IMPL-028','Reward allocator','impl','todo','P2',NULL,NULL,NULL,
  'specs/pve-empire.md',
  NULL,
  'Ordinary loot/debris plus campaign recognition; cap repeated contribution credit and exclude AI participation from human credit.'),
 (30,'DEF-004','Cooperative hostility extension point (E6/E7) + module policy registration','impl','todo','P2',NULL,NULL,NULL,
  'specs/module-extension-points.md (E6/E7); specs/pve-empire.md',
  'app/Contracts/HostilityPolicy.php; app/Services/HostilityGuard.php; app/GameMissions/Abstracts/GameMission.php; app/Http/Controllers/GalaxyController.php; Modules/AI/app/Support/CooperativeHostilityPolicy.php',
  'Host read-only guard (ordinary universe no-op; cooperative universe fail-closed with no policy or a throwing policy) enforced at the fleet and missile chokepoints; the module registers CooperativeHostilityPolicy, which blocks exactly coalition-vs-coalition hostility.'),
 (31,'IMPL-029','6C — fail-closed campaign consultation admission','impl','todo','P2',NULL,NULL,NULL,
  'WORK-PACKAGES.md (6C); specs/pve-empire.md',
  'config/campaign-consultation.php; app/Actions/ResolveCampaignConsultationAdmissionAction.php',
  'Admission core shipped: off/observe/advice mode (default off never resolves the SDK), staff switch, trigger allowlist, recorded stop reasons. Daily ceilings and per-trigger cooldown are enforced at the receipt path in IMPL-030; operator page/explain exposure rides with it.'),
 (32,'IMPL-030','6A — consultation transport + typed recommendation core','impl','todo','P2',NULL,NULL,NULL,
  'WORK-PACKAGES.md (6A); specs/pve-empire.md',
  'app/Ai/Agents/OgameCampaignConsultationAgent.php; app/Infrastructure/Language/LaravelAiCampaignConsultationGateway.php; app/Actions/RequestCampaignConsultationAction.php',
  'Shipped: the structured-output agent, the Null/Laravel AI gateways, the request/recommendation domain, the fail-closed admission wiring and the candidate-membership validator. Remaining 6A (brief builder, receipt+budget+caps, ranking adjustment) are IMPL-032/033/034.'),
 (33,'IMPL-031','6B — driver-signal utilisation for cooperative player divergence','impl','todo','P3',NULL,NULL,NULL,
  'WORK-PACKAGES.md (6B); specs/pve-empire.md',
  NULL,
  'One bounded driver-evidence contribution at existing campaign utility/social decision points; profile traits decide the reaction weight. Requires the 2/5/10 comparison showing a player-visible gain before any default.'),
 (34,'IMPL-032','6A — bounded redacted advice brief builder','impl','todo','P2',NULL,NULL,NULL,
  'WORK-PACKAGES.md (6A); specs/pve-empire.md',
  NULL,
  'Build the serialized redacted brief from current permitted campaign facts, legal executable candidates with native scores, and typed attributed driver evidence. Missing/invalid/stale/unauthorised driver evidence is absent.'),
 (35,'IMPL-033','6A — consultation receipt, budget reservation and caps','impl','todo','P2',NULL,NULL,NULL,
  'WORK-PACKAGES.md (6A); specs/pve-empire.md',
  NULL,
  'Persist a receipt per consultation (trigger, config revision, evidence IDs, provider/model, usage, validation result, changed ranking; never raw prompts or private facts). Reserve/settle usage through the existing budget; enforce daily ceilings, per-trigger cooldown and concurrency; uncertain usage settles once.'),
 (36,'IMPL-034','6A — profile-bounded ranking adjustment','impl','todo','P2',NULL,NULL,NULL,
  'WORK-PACKAGES.md (6A); specs/pve-empire.md',
  NULL,
  'Apply a validated recommendation as a profile-bounded adjustment to a later ranking; native policy still chooses and dispatches. No driver can promote an unavailable action or override a native refusal.');
...
INSERT OR REPLACE INTO dependencies (task_id, depends_on, reason) VALUES
 ...
 (28, 27, 'director resolves from objective completion'),
 (32, 31, 'fail-closed admission gates the consultation lane'),
 (34, 32, 'brief builder feeds the transport'),
 (35, 32, 'receipt/budget rides the transport'),
 (36, 34, 'adjustment consumes the brief and the candidate set'),
 (36, 35, 'adjustment settles after the receipt');
