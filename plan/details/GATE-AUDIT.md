...
3. **Host obligations — ten rules that live only in controllers (A3, B2, C3 and seven more).** The queue-upgrade predicate is the one this audit found; the capability map found nine others, including vacation mode blocking queue *additions*, the recall ownership check, and the expedition and fleet-speed bounds. The module keeps its restatement of the queue rule until the host publishes a predicate. They are asked for, in priority order with their minimal shapes, in [`specs/host-change-request.md`](specs/host-change-request.md).
