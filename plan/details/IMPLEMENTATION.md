...
This is the practical work map for the five roadmap phases. The original map used host `350657a3` / module `45f8a276`; the Phase 3 revision uses the [actual baseline](research/phase-3-current-state.md), host `02e23c13` / module `e1c48a2`. Phase 1/2 rows retain delivery context. Phase 3 slices 3A–3M are implemented; 3J's harness and export/delete acceptance are done and only its runs at 2/5/10 accounts remain, deferred by the owner to the very end of the phase and rescaled from 100/500/1,000 players on 14 September 2026, so a 10-account ordinary-play pilot stood in for them on 14 September 2026 and measured the population deciding but not acting. Treat the Phase 3 names below as delivered rather than proposed.
