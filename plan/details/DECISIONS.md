...
| The suite | **The generative switch is pinned off for tests, in the host suite configuration.** `Modules/AI/tests/Pest.php` is not the suite root for host runs -- the host suite points at `Modules/*/tests/Feature` -- so a hook written there is inert, and a run inherited `AI_LANGUAGE_ENABLED` from the developer's `.env`. Two tests that expect an authored reply had been passing for the wrong reason: their provider call failed, and that failure was what delivered the text they asserted. |
