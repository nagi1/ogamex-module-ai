...
| 3G — context and usage | **In progress:** native owner-scoped fact recall, deterministic bounded context selection, and atomic daily universe/account/conversation reservation ledgers are implemented; usage settles once without releasing the counted attempt. Provenance-rich sections remain. | Scoped/provenance-preserving context; protected-content overflow and concurrent cap tests. No provider needed. |
