...
| `LanguageGateway` | `generateConversationReply`: bounded authorized context + approved purpose + budget reservation → typed response and usage | Disabled default; one configured API provider for the first enabled deployment. |
...
| --- | --- | --- |
| 3A — source audit and observations | Confirm real host sources/schema/ACL; one committed source family with deduplication and legal reconciliation | Rollback, duplicate, late-source and recipient-isolation feature cases. No external dependency. |
| 3B — facts and obligations | Native fact/claim, relationship and commitment records plus exact retrieval and expiry | Attributed gossip, changed membership, exact terms, fulfilled/expired debt and deletion cases. |
| 3C — native affect | Small `AffectEngine`, goal/affect state, native appraisal, emotional episodes and disabled enrichment | Same evidence differs meaningfully by persona; state/decay/restart remain deterministic. |
| 3D — social protocols | `SocialCognition`, one request/response exchange family, native authored rules and dialogue state; add other exchange types in similarly small PRs | Accept/reject/counter/clarify, betrayal/apology, outstanding obligations and bounded AI-to-AI protocol tests. |
| 3E — outcome experience | One supported case family, native retention, `ExperienceEngine` and structured similarity | Actual outcome changes evidence; cold start, unknown features, failed cases, stale versions and duplicate retention. |
| 3F — authored human delivery | Pending replies/coalescing, route selection and permission-checked idempotent authored send | Real host persistence, ignore/membership changes, stale reply generation, duplicate send and crash reconciliation. LLM still disabled. |
| 3G — context and usage | `LongTermMemory` native recall, `ContextBuilder`, schemas and atomic reservation ledger | Scoped/provenance-preserving context; protected-content overflow and concurrent cap tests. No provider needed. |
| 3H — optional language | `LanguageGateway`, disabled implementation and one selected API adapter; combined proposals and validation | One-request semantics, real adapter boundary tests, invalid/truncated output, outage and usage reconciliation; opt-in quality evaluation. |
