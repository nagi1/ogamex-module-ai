...
5. **In progress — authorized 3H language slice.** The complete native social, experience, delivery, recall/context and budget matrix passed with provider-off behavior: Pint, Rector dry-run, module PHPStan, PAO Pest (138 tests, 550 assertions), exact 100% PCOV and TIA. The optional-provider phase is now explicitly authorized; retain that provider-off baseline while implementing and validating `LanguageGateway`.
...
| 3H — optional language | **In progress:** Laravel AI SDK host dependency is pinned at `laravel/ai` 0.8.1; module-owned language contract, typed disabled result and disabled default binding are in place. | Complete the selected-provider adapter, combined proposal validation, receipt lifecycle, SDK fake/real-adapter verification, failure reconciliation and opt-in quality evaluation. Follow [Laravel AI SDK integration](laravel-ai-sdk.md). |
