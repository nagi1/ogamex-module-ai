...
| Battle determinism | **yes (R1, 14 Sep 2026)** | `simulateBattle(?int $seed, bool $pure)` seeds the PHP engine's draws via `mt_srand`; `didSuccessfulRapidfire()` is now `mt_rand`, `rollMoonCreation()`/`checkHamillManoeuvre()` use a shared `random()`, `DefenseRepairService` gets a sub-seed | the **Rust FFI** round RNG is still its own — a seeded estimator must use `PhpBattleEngine` |
| Read-only simulation | **yes (R1, 14 Sep 2026)** | `simulateBattle(seed, pure: true)` suppresses `applyTacticalRetreat()`'s resource write and the `BattleResolved` event | only those two side effects existed; the rest of the run is already in-memory |
