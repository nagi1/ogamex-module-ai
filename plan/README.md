...
**Current work: the disclosed pilot.** Packages 1–4 are implemented. Package 3 delivers slices 3A–3L: deterministic social cognition, experience, memory, the conversation cycle and the bounded provider escalation, which is off by default. Package 4 gives operators the caps, the staff switch, decision inspection, a read-only scenario replay, production-refusing seeding and a pilot report on the existing admin page. What remains is evidence rather than code: 3J's 100/500/1,000-player load runs, 3H's real-provider conformance artifact, Gate 2 for the three opt-in drivers, and the pilot itself. Do not repeat completed Package 1/2 work, and do not treat the load measurements as already proven.
