...
**Wave 10 is the session-cost read (17 September 2026), filed but not yet confirmed.** A live AI session job measures 1–36 s where its sibling work kinds measure ~310 ms, and the read attributes ~297 of a session's ~430 queries to `GameMissionFactory::getAllMissions()` being instantiated inside `FacilityChain` to read metadata that is already `static`. Both rows — [W10-1 and W10-2](details/GAP-REGISTER.md#wave-10--session-cost-read-17-september-2026) — carry their fix: `REV-005` reproduces the figures first on the committed build, and `IMPL-043` (P1, the mission catalogue) and `IMPL-044` (P2, one player reload per job) both depend on it. The `CACHE_STORE` misconfiguration and the 405-query colony walk found in the same pass are already fixed in the working tree and are recorded in the wave's preamble so the rows are read against the right baseline.
