...
**Current work: Package 4, operability and disclosed pilot.** Slices 3A–3L are implemented and committed for Package 3: the deterministic conversation cycle answers a real message in ordinary play, and a substantive reply may be handed to the language lane, which is off by default. 3J's scenario/ablation harness and export/delete acceptance are implemented; its 100/500/1,000-player load runs are deferred by the owner to the end of the phase. Three items stay operator-run or evidence-gated: the language slice needs its real-provider conformance artifact and human review, Gate 2 for both real drivers needs held-out-outcome evidence before either is enabled, and the load measurements are not taken yet. Do not repeat completed Package 1/2 work, and do not treat the load measurements as already proven.
